...
#' @param drat_repo a
#' @export
source_to_binary <- function(packages_to_build, drat_repo){
  ...
  for(i in seq_along(pkg_srcs)){
    devtools::install_local(file.path(drat_repo,"src","contrib",pkg_srcs[i]), repos = 'https://cran.rstudio.com', force=T, upgrade = "always")
